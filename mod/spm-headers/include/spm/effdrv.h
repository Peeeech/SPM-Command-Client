...
param_4 - type(?)
    0: flashes whole screen with white/inverted color then disappears
    1: spawns 'electricity star' effect on mario
param_5 - count(?)
    x: seems to dictate how many times the effect spawns, with a delay between each spawn. 
param_6 - time(?)
    x: seems to dictate how fast the effect spawns, with a delay between each spawn. Higher is slower.
param_7 - unk
param_8 - unk
*/
EffEntry * effItemThunderEntry(float x, float y, float z, s32 param_4, s32 param_5, s32 param_6, s32 param_7, s32 param_8);

/* effDamageStarEntry
userWork = size(0x24)
damageType (0 or 1) 
    0 = red bubble( Mario taking damage)
    1 = yellow bubble( enemy taking damage)
s32 displayed damage amount as an int
*/ 
EffEntry * effDamageStarEntry(float x, float y, float z,int damageType, s32 damage);
...
EffEntry* unk_800a845c(s32 type);
void unk_80061a18(EffEntry* entry, int blob, char name);
